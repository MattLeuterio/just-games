const colors = {
  primary: {
    white: '#ffffff',
    black: '#100E16',
    gray: '#C6C6C6',
    lightGray: '#96A7AF',
    primary: '#8E2DE0',
    transparentPrimary: 'rgb(32 32 32 / 55%)',
    secondary: '#4A00E0',
    red: '#EB5757',
    green: '#27AE60',
    light: '#ECEAEF',
    lightAccent: '#624898',
    dark: '#26203B',
    darkAccent: '#71706E',
  },
  vote: {
    red: "#e23132",
    orange: "#e87336",
    greenLight: "#acb85b",
    greenDark: "#acb85b",
    green: "#49bc8a"
  },
  gradients: {
    primary: 'linear-gradient(109.08deg, #8E2DE0 12.85%, #4A00E0 87.15%)',
    primaryTransparent: 'linear-gradient(109.08deg,#8e2de0c4 12.85%,#4a00e0fa 87.15%)',
    blue: 'linear-gradient(180deg, rgba(74, 0, 224, 0.51) 0%, #4A00E0 100%)',
    card: 'linear-gradient(180deg, rgba(32, 32, 32, 0.55) 0%, rgba(14, 14, 14, 0.78) 100%)',
    navbar: 'linear-gradient(180deg, #4A00E0 0%, rgba(74, 0, 224, 0) 69.01%)',
    bg: 'linear-gradient(180deg, #8E2DE0 0%, rgba(142, 45, 224, 0) 100%)',
    gameHeader: 'linear-gradient(179.83deg,rgba(0,0,0,0.6) 54.31%,#4A00E0 83.33%)',
  }
};

export default colors;